...
open class MarqueeLabel: UILabel, CAAnimationDelegate {
      ...
      if tapToScroll != oldValue {
        if tapToScroll {
          let tapRecognizer = UITapGestureRecognizer(
            target: self, action: #selector(MarqueeLabel.labelWasTapped(_:)))
  ...
  private func setup() {
    ...
    NotificationCenter.default.addObserver(
      self, selector: #selector(MarqueeLabel.restartForViewController(_:)),
      name: NSNotification.Name(rawValue: MarqueeKeys.Restart.rawValue), object: nil)
    NotificationCenter.default.addObserver(
      self, selector: #selector(MarqueeLabel.labelizeForController(_:)),
      name: NSNotification.Name(rawValue: MarqueeKeys.Labelize.rawValue), object: nil)
    NotificationCenter.default.addObserver(
      self, selector: #selector(MarqueeLabel.animateForController(_:)),
      name: NSNotification.Name(rawValue: MarqueeKeys.Animate.rawValue), object: nil)
    // UIApplication state notifications
    NotificationCenter.default.addObserver(
      self, selector: #selector(MarqueeLabel.restartLabel), name: UIApplication.didBecomeActiveNotification,
      object: nil)
    NotificationCenter.default.addObserver(
      self, selector: #selector(MarqueeLabel.shutdownLabel),
  ...
  private func updateAndScroll(overrideHold: Bool) {
    ...
    if !labelShouldScroll() {
      ...
      default:
        labelFrame =
          CGRect(
            x: leadingBuffer, y: 0.0, width: bounds.size.width - leadingBuffer, height: bounds.size.height
          ).integral
      }
    ...
    case .continuous, .continuousReverse:
      if type == .continuous {
        homeLabelFrame =
          CGRect(x: leadingBuffer, y: 0.0, width: expectedLabelSize.width, height: bounds.size.height)
          .integral
        awayOffset = -(homeLabelFrame.size.width + minTrailing)
      } else {  // .ContinuousReverse
        homeLabelFrame =
          CGRect(
            x: bounds.size.width - (expectedLabelSize.width + leadingBuffer), y: 0.0,
            width: expectedLabelSize.width, height: bounds.size.height
          ).integral
        awayOffset = (homeLabelFrame.size.width + minTrailing)
      }
      ...
      let startFadeTimeFraction = timingFunctionForAnimationCurve(animationCurve)
        .durationPercentageForPositionPercentage(
          startFadeFraction, duration: (animationDelay + animationDuration))
      ...
      sequence =
        scrollSequence ?? [
          ...
          ScrollStep(
            timeStep: animationDuration, timingFunction: animationCurve,  // Ending point (back at home), with animationCurve transition, with trailing fade
      ...
      if type == .leftRight || type == .rightLeft {
        sequence =
          scrollSequence ?? [
            ...
            ScrollStep(
              timeStep: animationDuration, timingFunction: animationCurve,  // Away position, using animationCurve transition, with only leading edge faded in
            ...
            ScrollStep(
              timeStep: animationDuration, timingFunction: animationCurve,  // Ending point, back at home, with only trailing fade
  ...
  open func labelShouldScroll() -> Bool {
    ...
    if !super.adjustsFontSizeToFitWidth {
      ...
      labelTooLarge =
        (sublabel.desiredSize().width + leadingBuffer) > self.bounds.size.width + CGFloat.ulpOfOne
    } else {
      ...
      resizedString.enumerateAttribute(.font, in: NSRange(0..<sublabel.attributedText!.length)) {
        val, rng, stop in
  ...
  private func transactionDurationType(_ labelType: MarqueeType, interval: CGFloat, delay: CGFloat)
    -> TimeInterval
  {
    switch labelType {
  ...
  class fileprivate func notifyController(_ controller: UIViewController, message: MarqueeKeys) {
    NotificationCenter.default.post(
      name: Notification.Name(rawValue: message.rawValue), object: nil, userInfo: ["controller": controller])
  }
  ...
  open func textCoordinateForFramePoint(_ point: CGPoint) -> CGPoint? {
    ...
    guard
      let container = containers.filter({ (rng) -> Bool in
...
public struct ScrollStep: MarqueeStep {
  ...
  public init(
    timeStep: CGFloat, timingFunction: UIView.AnimationCurve = .linear, position: Position,
    edgeFades: EdgeFade
  ) {
...
extension UILabel {
  fileprivate func desiredSize() -> CGSize {
...
extension Range where Bound == CGFloat {
  fileprivate func rangeForExtension(_ ext: CGFloat) -> Range {
...
extension UIResponder {
  ...
  fileprivate func firstAvailableViewController() -> UIViewController? {
    // convenience function for casting and to "mask" the recursive function
    return self.traverseResponderChainForFirstViewController()
  }

  fileprivate func traverseResponderChainForFirstViewController() -> UIViewController? {
...
extension CAMediaTimingFunction {
  ...
  fileprivate func durationPercentageForPositionPercentage(_ positionPercentage: CGFloat, duration: CGFloat)
    -> CGFloat
  {
  ...
  fileprivate func solveTforY(_ y_0: CGFloat, epsilon: CGFloat, controlPoints: [CGPoint]) -> CGFloat {
  ...
  fileprivate func YforCurveAt(_ t: CGFloat, controlPoints: [CGPoint]) -> CGFloat {
  ...
  fileprivate func XforCurveAt(_ t: CGFloat, controlPoints: [CGPoint]) -> CGFloat {
    let P0 = controlPoints[0]
    let P1 = controlPoints[1]
    let P2 = controlPoints[2]
    let P3 = controlPoints[3]
  ...
  fileprivate func derivativeCurveYValueAt(_ t: CGFloat, controlPoints: [CGPoint]) -> CGFloat {
    let P0 = controlPoints[0]
    let P1 = controlPoints[1]
    let P2 = controlPoints[2]
    let P3 = controlPoints[3]
  ...
  fileprivate func controlPoints() -> [CGPoint] {
